...
require_once(dirname(dirname(__DIR__)) . '/classes/ap-settings.php');

/**
 * Unit tests for the AP_Settings class
 *
 * Uses the SimpleTest For WordPress plugin
 *
 * @link http://wordpress.org/extend/plugins/simpletest-for-wordpress/
 */
if ( ! class_exists( 'UnitTestAP_Settings' ) ) {
	class UnitTestAP_Settings extends UnitTestCase {
		public function __construct() {
			$this->AP_Settings = AP_Settings::get_instance();
		}
		...
		public function test_validate_settings() {
			...
			$clean_settings = $this->AP_Settings->validate_settings( $valid_settings );
			...
			$clean_settings = $this->AP_Settings->validate_settings( $invalid_settings );
		...
		public function test_magic_set() {
			...
			$this->AP_Settings->init();
			$this->AP_Settings->settings = array( 'db-version' => array() );
			$this->assertEqual( $this->AP_Settings->settings['db-version'], AvanPress::VERSION );

			// Test that values gets written to database
			$this->AP_Settings->settings = array( 'db-version' => '5' );
			$this->AP_Settings->init();
			$this->assertEqual( $this->AP_Settings->settings['db-version'], '5' );
			$this->AP_Settings->settings = array( 'db-version' => AvanPress::VERSION );
			...
			$this->AP_Settings->settings['db-version'] = AvanPress::VERSION;
		}
	} // end UnitTestAP_Settings
}
