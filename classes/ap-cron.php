...
if ( ! class_exists( 'AP_Cron' ) ) {
	 ...
	 */
	class AP_Cron extends AP_Module {
		...
		public static function add_custom_cron_intervals( $schedules ) {
			$schedules[ 'ap_debug' ] = array(
				'interval' => 5,
				'display'  => 'Every 5 seconds'
			);

			$schedules[ 'ap_ten_minutes' ] = array(
				'interval' => 60 * 10,
				'display'  => 'Every 10 minutes'
			);

			$schedules[ 'ap_example_interval' ] = array(
				'interval' => 60 * 60 * 5,
		...
		public static function fire_job_at_time() {
			...
			if ( (int) date( 'G', $now ) >= 1 && (int) date( 'G', $now ) <= 3 ) {
				if ( ! get_transient( 'ap_cron_example_timed_job' ) ) {
					//AP_CPT_Example::exampleTimedJob();
					set_transient( 'ap_cron_example_timed_job', true, 60 * 60 * 6 );
				}
			}
		}
		...
		public function activate( $network_wide ) {
			if ( wp_next_scheduled( 'ap_cron_timed_jobs' ) === false ) {
				wp_schedule_event(
			...
			if ( wp_next_scheduled( 'ap_cron_example_job' ) === false ) {
				wp_schedule_event(
					current_time( 'timestamp' ),
					'ap_example_interval',
					'ap_cron_example_job'
		...
		public function deactivate() {
			wp_clear_scheduled_hook( 'ap_cron_timed_jobs' );
			wp_clear_scheduled_hook( 'ap_cron_example_job' );
		}
